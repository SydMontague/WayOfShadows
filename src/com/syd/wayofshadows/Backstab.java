...
public class Backstab implements Listener
{
    ...
    private boolean hasEPerm(Player damager, PotionEffectType type, int id)
    {
        if (damager.hasPermission("shadow.effect") || damager.hasPermission("shadow.effect." + id) || damager.hasPermission("shadow.effect." + id + "." + type.getName()))
            return true;
        else
            return false;
    }
    
    private boolean hasBPerm(Player damager, int id)
    {
        if (damager.hasPermission("shadow.backstab") || damager.hasPermission("shadow.backstab." + id))
            return true;
        else
            return false;
    }
    
    private double getAngle(Vector vec1, Vector vec2)
    {
